...
u8 *config_dir = ".";              //< Directory path with the target analysis artifacts
...
u8 cooling_schedule = 0;           //< Cooling schedule for directed fuzzing

enum {
    SAN_EXP,                       //< Exponential schedule
    SAN_LOG,                       //< Logarithmic schedule
    SAN_LIN,                       //< Linear schedule
    SAN_QUAD                       //< Quadratic schedule
};
...
s32 out_fd,                        //< Persistent fd for out_file
    ...
    fsrv_ctl_fd2, fsrv_st_fd2;

s32 forksrv_pid,                   //< PID of the fork server
    forksrv_pid2, child_pid = -1,  //< PID of the fuzzed program
    child_pid2 = -1,
...
struct queue_entry *top_conformance[MAP_SIZE]; //< Top entries for bitmap bytes
...
struct queue_entry *top_cb_rated[MAP_SIZE];
u8 cb_score_changed;
...
u8 solved_cbbs[MAP_SIZE];
// u32 distance_val[MAP_SIZE];
...
u8*  condition_bits;               //< Condition coverage status
u64* condition_vars;               //< Condition variable values
u64  condition_vals[2 * MAP_SIZE]; //< Condition constant values

u8 solved_cnds[MAP_SIZE];

u32 cnd_id_map[MAP_SIZE];

u32 *critical_ids;
u32 critical_count[MAP_SIZE];
...
u32 period_critical_count;

bool explore_status = false;
...
tbb_info_t **tbb_infos = NULL;     //< 1D array containing target BB information

int32_t **distance_matrix = NULL;  //< 2D array containing the distance of each critical BB to each target BB (-1 if unreachable)
float *cbb_distances = NULL;       //< 1D array containing for each critical BB the average distance to all reachable target BBs
// clang-format on
...
/* Scale a value to a different value range */
...
bool hit_rare_targets(struct queue_entry *q) {
...
const u8 count_class_lookup8[256] = {[0] = 0,          [1] = 1,           [2] = 2,
                                     [3] = 4,          [4 ... 7] = 8,     [8 ... 15] = 16,
...
void remove_shm(void) { shmctl(shm_id, IPC_RMID, NULL); }
...
void update_cb_bitmap_score(struct queue_entry *q) {
    u32 i;
    u64 fav_factor = q->exec_us * q->len;
...
void cull_queue(void) {
    ...
    for (i = 1; i < n_cnds; i++) {
        if (top_conformance[i] && (solved_cnds[i] < 3)) {
...
void cb_cull_queue(void) {
    struct queue_entry *q;

    u32 i;

    if (dumb_mode || !cb_score_changed) {
        return;
    }
...
void setup_shm(void) {
    ...
    if (!dumb_mode) {
        setenv("__CRITICAL_BB_ID", shm_str2, 1);
    }
    ...
    if (!dumb_mode) {
        setenv("__DISTANCE_BB_ID", shm_str3, 1);
    }
    ...
    s32 shm_id4 = shmget(IPC_PRIVATE, sizeof(u64) * n_cnds * 2, IPC_CREAT | IPC_EXCL | 0600);
    ...
    if (!dumb_mode) {
        setenv(CONDITION_VAR, shm_str4, 1);
    }

    ck_free(shm_str4);

    condition_vars = shmat(shm_id4, NULL, 0);

    if (!condition_vars) {
        PFATAL("shmat() failed");
    }

    s32 shm_id5 = shmget(IPC_PRIVATE, n_cnds, IPC_CREAT | IPC_EXCL | 0600);
    ...
    if (!dumb_mode) {
        setenv(COND_ENV_VAR, shm_str5, 1);
    }

    ck_free(shm_str5);

    condition_bits = shmat(shm_id5, NULL, 0);
...
void init_forkserver2(char **argv) {
    ...
    if (!forksrv_pid2) {
        ...
        if (dup2(ctl_pipe[0], FORKSRV_FD2) < 0) {
            PFATAL("dup2() failed");
        }
        if (dup2(st_pipe[1], FORKSRV_FD2 + 1) < 0) {
            PFATAL("dup2() failed");
        }
...
u8 run_target(char **argv, u32 timeout) {
    ...
    memset(condition_vars, 0, sizeof(u64) * n_cnds * 2);
    memset(condition_bits, 0, n_cnds);
...
u8 run_target2(char **argv, u32 timeout) {
    static struct itimerval it;
    static u32 prev_timed_out = 0;
...
void cnd_stat(void) {
    ...
    for (i = 1; i < n_cnds; i++) {
        if (condition_bits[i]) {
            solved_cnds[i] |= condition_bits[i];
        }
    }
}

u32 num_equal_bits(u64 l, u64 r) {
    u32 count = 0;
    u64 res = ~(l ^ r);
    while (res != 0) {
        count += (res & 1);
...
bool has_new_critical_conformance() {
    for (u32 i = 0; i < critical_ids[0]; i++) {
        u32 cbb_id = critical_ids[i + 1];
        u32 cnd_id = cnd_id_map[cbb_id];

        if (unlikely(cnd_id) && (!solved_cbbs[cbb_id])) {
            u64 conformance = 0;
...
void update_critical_conformance() {
    for (u32 i = 0; i < critical_ids[0]; i++) {
        u32 cbb_id = critical_ids[i + 1];
        u32 cnd_id = cnd_id_map[cbb_id];
        ...
        if (unlikely(cnd_id) && (!solved_cbbs[cbb_id])) {
            ...
            if (conformance > condition_vals[2 * cnd_id]) {
                condition_vals[2 * cnd_id] = conformance;

                if (top_conformance[cnd_id]) {
...
u8 save_if_interesting(char **argv, void *mem, u32 len, u8 fault) {
    ...
    if (fault == crash_mode) {
        ...
        fn = alloc_printf("%s/queue/id:%06u,%llu,%s", out_dir, queued_paths, get_cur_time() - start_time,
                          describe_op(hnb));
    ...
    case FAULT_TMOUT:
        ...
        fn = alloc_printf("%s/hangs/id:%06llu,%llu,%s", out_dir, unique_hangs, get_cur_time() - start_time,
                          describe_op(0));
    ...
    keep_as_crash:
        ...
        fn = alloc_printf("%s/crashes/id:%06llu,%llu,sig:%02u,%s", out_dir, unique_crashes, get_cur_time() - start_time,
...
u8 save_if_crash(char **argv, void *mem, u32 len, u8 fault) {
    u8 *fn = "";
    s32 fd;
    u8 saved = 0;
...
void write_stats_file(double bitmap_cvg, double stability, double eps) {
    ...
    fprintf(f,
            ...
            no_forkserver ? "no_forksrv " : "", crash_mode ? "crash " : "", persistent_mode ? "persistent " : "",
...
void maybe_delete_out_dir(void) {
    ...
    if (in_place_resume && rmdir(fn)) {
        ...
        u8 *nfn = alloc_printf("%s.%04u-%02u-%02u-%02u:%02u:%02u", fn, t->tm_year + 1900, t->tm_mon + 1, t->tm_mday,
...
void show_stats(void) {
    ...
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
            crash_mode ? cPIN "peruvian were-rabbit" : cYEL "SASTFuzz", use_banner);
...
#define SP20 SP10 SP10
    ...
    SAYF(SET_G1 bSTG bLT bH bSTOP cCYA " process timing " bSTG bH30 bH5 bH2 bHB bH bSTOP cCYA
    ...
    sprintf(tmp, "%s%s (%0.02f%%)", DI(current_entry), queue_cur->favored ? "" : "*",
    ...
    SAYF(bVR bH cCYA bSTOP " fuzzing strategy yields " bSTG bH10 bH bHT bH10 bH5 bHB bH bSTOP cCYA
    ...
    if (skip_deterministic) {
        ...
        sprintf(tmp, "%s/%s, %s/%s, %s/%s", DI(stage_finds[STAGE_FLIP1]), DI(stage_cycles[STAGE_FLIP1]),
                DI(stage_finds[STAGE_FLIP2]), DI(stage_cycles[STAGE_FLIP2]), DI(stage_finds[STAGE_FLIP4]),
    ...
    if (!skip_deterministic) {
        sprintf(tmp, "%s/%s, %s/%s, %s/%s", DI(stage_finds[STAGE_FLIP8]), DI(stage_cycles[STAGE_FLIP8]),
                DI(stage_finds[STAGE_FLIP16]), DI(stage_cycles[STAGE_FLIP16]), DI(stage_finds[STAGE_FLIP32]),
                DI(stage_cycles[STAGE_FLIP32]));
    }

    SAYF(bV bSTOP "  byte flips : " cRST "%-37s " bSTG bV bSTOP "   pending : " cRST "%-10s " bSTG bV "\n", tmp,
         DI(pending_not_fuzzed));

    if (!skip_deterministic) {
        sprintf(tmp, "%s/%s, %s/%s, %s/%s", DI(stage_finds[STAGE_ARITH8]), DI(stage_cycles[STAGE_ARITH8]),
                DI(stage_finds[STAGE_ARITH16]), DI(stage_cycles[STAGE_ARITH16]), DI(stage_finds[STAGE_ARITH32]),
    ...
    if (!skip_deterministic) {
        sprintf(tmp, "%s/%s, %s/%s, %s/%s", DI(stage_finds[STAGE_INTEREST8]), DI(stage_cycles[STAGE_INTEREST8]),
                DI(stage_finds[STAGE_INTEREST16]), DI(stage_cycles[STAGE_INTEREST16]),
                DI(stage_finds[STAGE_INTEREST32]), DI(stage_cycles[STAGE_INTEREST32]));
    }

    SAYF(bV bSTOP "  known ints : " cRST "%-37s " bSTG bV bSTOP " own finds : " cRST "%-10s " bSTG bV "\n", tmp,
         DI(queued_discovered));

    if (!skip_deterministic) {
        sprintf(tmp, "%s/%s, %s/%s, %s/%s", DI(stage_finds[STAGE_EXTRAS_UO]), DI(stage_cycles[STAGE_EXTRAS_UO]),
                DI(stage_finds[STAGE_EXTRAS_UI]), DI(stage_cycles[STAGE_EXTRAS_UI]), DI(stage_finds[STAGE_EXTRAS_AO]),
    ...
    sprintf(tmp, "%s/%s, %s/%s", DI(stage_finds[STAGE_HAVOC]), DI(stage_cycles[STAGE_HAVOC]),
    ...
    SAYF(" stability : %s%-10s " bSTG bV "\n",
         (stab_ratio < 85 && var_byte_count > 40)
                 ? cLRD
                 : ((queued_variable && (!persistent_mode || var_byte_count > 20)) ? cMGN : cRST),
         tmp);
...
void show_init_stats(void) {
    ...
    OKF("Here are some useful stats:\n\n"
        ...
        ((double)total_bitmap_size) / (total_bitmap_entries ? total_bitmap_entries : 1), DI(min_us), DI(max_us),
        DI(avg_us));
...
u32 get_random_modifiable_posn(u32 num_to_modify, u8 mod_type, u32 map_len, u8 *branch_mask, u32 *position_map) {
    u32 ret = 0xffffffff;
...
u32 get_random_insert_posn(u32 map_len, u8 *branch_mask, u32 *position_map) {
    u32 position_map_len = 0;
    u32 ret = map_len;
...
u8 fuzz_one(char **argv) {
    ...
    if (not_on_tty) {
        ACTF("Fuzzing test case #%u (%u total, %llu uniq crashes found)...", current_entry, queued_paths,
             unique_crashes);
        fflush(stdout);
    }
    ...
    if ((skip_deterministic && (!target_fast || !queue_cur->is_target)) || queue_cur->was_fuzzed ||
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l)-1) - EFF_APOS(_p) + 1)
    ...
    for (i = 0; i < len - 3; i++) {
        /* Let's consult the effector map... */
        if (!eff_map[EFF_APOS(i)] && !eff_map[EFF_APOS(i + 1)] && !eff_map[EFF_APOS(i + 2)] &&
            !eff_map[EFF_APOS(i + 3)]) {
            stage_max--;
            continue;
        }
...
skip_bitflip:
    ...
    for (i = 0; i < len - 1; i++) {
        ...
        for (j = 1; j <= ARITH_MAX; j++) {
            u16 r1 = orig ^ (orig + j), r2 = orig ^ (orig - j), r3 = orig ^ SWAP16(SWAP16(orig) + j),
    ...
    for (i = 0; i < len - 3; i++) {
        ...
        for (j = 1; j <= ARITH_MAX; j++) {
            u32 r1 = orig ^ (orig + j), r2 = orig ^ (orig - j), r3 = orig ^ SWAP32(SWAP32(orig) + j),
...
skip_arith:
    ...
    for (i = 0; i < len - 3; i++) {
        ...
        for (j = 0; j < sizeof(interesting_32) / 4; j++) {
            ...
            if (!could_be_bitflip(orig ^ (u32)interesting_32[j]) && !could_be_arith(orig, interesting_32[j], 4) &&
...
skip_interest:
    ...
    for (i = 0; i < len; i++) {
        ...
        for (j = 0; j < extras_cnt; j++) {
            ...
            if ((extras_cnt > MAX_DET_EXTRAS && UR(extras_cnt) >= MAX_DET_EXTRAS) || extras[j].len > len - i ||
...
skip_user_extras:
    ...
    for (i = 0; i < len; i++) {
        ...
        for (j = 0; j < MIN(a_extras_cnt, USE_AUTO_EXTRAS); j++) {
            ...
            if (a_extras[j].len > len - i || !memcmp(a_extras[j].data, out_buf + i, a_extras[j].len) ||
    ...
    for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
        ...
        for (i = 0; i < use_stacking; i++) {
            ...
            case 11 ... 12: {
                ...
                memmove(cb_mask + del_from, cb_mask + del_from + del_len, temp_len - del_from - del_len + 1);
            ...
            case 13:
                ...
                if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
                    ...
                    memcpy(new_cb_mask + clone_to + clone_len, cb_mask + clone_to, temp_len - clone_to + 1);
            ...
            case 16: {
                ...
                memcpy(new_cb_mask + insert_at + extra_len, cb_mask + insert_at, temp_len - insert_at + 1);
...
void sync_fuzzers(char **argv) {
    ...
    while ((sd_ent = readdir(sd))) {
        ...
        while ((qd_ent = readdir(qd))) {
            ...
            if (qd_ent->d_name[0] == '.' || sscanf(qd_ent->d_name, CASE_PREFIX "%06u", &syncing_case) != 1 ||
...
void handle_skipreq(int sig) { skip_requested = 1; }
...
void check_binary2(u8 *fname) {
    ...
    if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
        ...
        if (stat(target_path2, &st) || !S_ISREG(st.st_mode) || !(st.st_mode & 0111) || (f_len = st.st_size) < 4) {
            FATAL("Program '%s' not found or not executable", fname);
        }
    ...
    if (fd < 0) {
        PFATAL("Unable to open '%s'", target_path2);
    }

    f_data = mmap(0, f_len, PROT_READ, MAP_PRIVATE, fd, 0);

    if (f_data == MAP_FAILED) {
        PFATAL("Unable to mmap file '%s'", target_path2);
    }
...
void get_core_count(void) {
    ...
    if (cpu_core_count > 0) {
        ...
        OKF("You have %u CPU core%s and %u runnable tasks (utilization: %0.0f%%).", cpu_core_count,
            cpu_core_count > 1 ? "s" : "", cur_runnable, cur_runnable * 100.0 / cpu_core_count);
...
void readCondition() {
    ...
    while (fgets(buf, sizeof(buf), condition_file) != NULL) {
        ...
        token = strtok(buf, " ");
        u32 cnd_id = atoi(token);

        token = strtok(NULL, " ");
        if (strcmp(token, "none") != 0) {
            u32 cbb_id = atoi(token);
            cnd_id_map[cbb_id] = cnd_id;
        }

        token = strtok(NULL, " ");  // 3th item
        token = strtok(NULL, " ");  // 4th item
        if (strcmp(token, "int32") == 0) {
            condition_info[cnd_id] |= 4;
        }
        if (strcmp(token, "int64") == 0) {
            condition_info[cnd_id] |= 8;
        }

        token = strtok(NULL, " ");  // 5th item
        ...
        if (strcmp(token, "var") != 0) {
            // Record the value of constant 2
            condition_info[cnd_id] |= 1;
            u64 value = strtoul(token, 0, 10);
            condition_vals[2 * cnd_id + 1] = value;
        }
        ...
        case 'M': { /* master sync ID */
            ...
            if ((c = strchr(sync_id, ':'))) {
                *c = 0;
                ...
                if (sscanf(c + 1, "%u/%u", &master_id, &master_max) != 2 || !master_id || !master_max ||
                    master_id > master_max || master_max > 1000000) {
